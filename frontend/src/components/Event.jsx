...
export default function Event({ state, dispatch, ACTIONS }) {
  ...
  const {
    ...
    const options = JSON.parse(getById(eventId, state.events).options).map(option => <button onClick={() => {console.log(option.nextEvent); dispatch({ type: ACTIONS.NEXT_EVENT, value: option.nextEvent })}}>{option.text}</button>);
